...
class ComputerEngineer:
    def __init__(self):
        self.name = "Milad Rezaee"
        self.role = "Computer Engineering Student"
        self.university = "Amir Kabir University of Technology"
    ...
    def code(self, languages: List[str]) -> None:
        print(f"💻 Coding in {', '.join(languages)}")

    def read(self) -> None:
        print("📖 Expanding knowledge through books")

    def daily_routine(self) -> None:
        while True:
            self.study()
            self.code(self.languages)
            self.read()
            time.sleep(8)  # Rest is important! 😴
